...
class DepositZK extends Component {
  constructor(props) {
    super(props);
    reactAutobind(this);
  }
        ...
        <View
          ...
          <View>
            <Text style={GlobalStyles.exoTitle}>
              Receive {EVM.network} native{'\n'}or ERC-20 Tokens
            </Text>
          </View>
          <QRCodeStyled
            maxSize={Dimensions.get('screen').width * (ratio > 1.7 ? 0.8 : 0.5)}
            data={this.context.value.zkPublicKey}
            style={[
              {
          ...
          <View
            ...
            <Text
              ...
              {this.context.value.zkPublicKey.substring(0, 21) +
                '\n' +
                this.context.value.zkPublicKey.substring(21)}
            </Text>
            <Pressable
              onPress={() => {
                Clipboard.setString(this.context.value.zkPublicKey);
                ToastAndroid.show(
